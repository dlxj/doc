...
namespace AStar
{
	...
	{
		int idx = 0;
		int len = pathLength[pathfinderID];
		paths.steps = (step*)malloc(len * sizeof(step));
		paths.lenght = len;
						...
						if (a != -1 && b != -1 && a != mapWidth && b != mapHeight) {
							...
							if (whichList[a][b] != onClosedList) {
								...
								if (walkability[a][b] != unwalkable) {
									...
									if (a == parentXval - 1)
									{
										if (b == parentYval - 1)		// parentµÄ×óÉÏ½Ç·½¿é
										{
											if (walkability[parentXval - 1][parentYval] == unwalkable
												|| walkability[parentXval][parentYval - 1] == unwalkable) \
												// parentµÄÓÒ±ß»òÉÏ±ß·½¿éÊÇÇ½£¬
												// Ôò²»ÄÜ´Óparentµ½´ïËüµÄÕâ¸öÁÚ½ü·½¿é(×óÉÏ½Ç·½¿é)
												corner = unwalkable;
										}
										else if (b == parentYval + 1)  // parentµÄ×óÏÂ½Ç·½¿é
										{
											if (walkability[parentXval][parentYval + 1] == unwalkable
												|| walkability[parentXval - 1][parentYval] == unwalkable)
												// parentµÄÏÂ±ß»ò×ó±ß·½¿éÊÇÇ½£¬
												// Ôò²»ÄÜ´Óparentµ½´ïËüµÄÕâ¸öÁÚ½ü·½¿é(×óÏÂ½Ç·½¿é)
												corner = unwalkable;
										}
									}
									else if (a == parentXval + 1)
									{
										if (b == parentYval - 1)      // parentµÄÓÒÉÏ½Ç·½¿é
										{
											if (walkability[parentXval][parentYval - 1] == unwalkable
												|| walkability[parentXval + 1][parentYval] == unwalkable)
												corner = unwalkable;
										}
										else if (b == parentYval + 1)  // parentµÄÓÒÏÂ½Ç·½¿é
										{
											if (walkability[parentXval + 1][parentYval] == unwalkable
												|| walkability[parentXval][parentYval + 1] == unwalkable)
												corner = unwalkable;

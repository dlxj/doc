# -*- coding: utf-8 -*-
"""
Created on Fri Dec  7 14:27:49 2018
...
ax.text(-0.6, 4.4, "非线性模型",fontsize=AXIS_LABEL_FONT_SIZE , fontproperties=myfont)
...
data = np.concatenate([data, 
                       (data[:,0] ** 2).reshape(60, -1), 
                       (data[:,1] ** 2).reshape(60, -1), 
                       (data[:,0] * data[:, 1]).reshape(60, -1)
                       ], axis=1)
label = [0] * (number_of_points // 2) + [1] * (number_of_points // 2)
lr = LogisticRegression().fit(data, label)
...
x1, x2 = np.meshgrid(x1, x2)
x1, x2 = x1.flatten(), x2.flatten()
x3 = lr.predict_proba(np.array([x1, x2, x1**2, x2**2, x1*x2]).transpose())[:,1]
ax.contourf(x1.reshape(step,-1), x2.reshape(step,-1), x3.reshape(step,-1), cmap=plt.cm.Greys, alpha=ALPHA)

plt.savefig(os.path.join(all_pic_path, '5-15.png'), format='png')
...
for c,eta, param, name, algo in zip(np.arange(len(names)), etas, params, names, algos):
    ...
    ax.text(p[0], p[1] , bottom + 0.1, r"$x^{start}$", fontsize=TEXT_FONT_SIZE)
    tr1 = [p[0]]
    tr2 = [p[1]]
    ...
    ax.scatter(p[0], p[1], bottom, s=POINT_SIZE, c="k")
    ax.scatter(p[0], p[1], wierd(p), s=POINT_SIZE, c="k")
    ax.text(p[0], p[1] , bottom + 0.1, r"$x^{end}$", fontsize=TEXT_FONT_SIZE)
...
plt.savefig(os.path.join(all_pic_path, '3-19.png'), format='png')
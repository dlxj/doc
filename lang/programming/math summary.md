...
- https://www.zhihu.com/question/50390618/answer/1298216571
  - **纯数是数无单位，无量纲量是量有单位。**
  - 实际上，**角度是量，是量就有单位**。考虑一个扇形有1m的弧长和1m的半径，弧长和半径的比值是纯数1，此时扇形有圆心角，定义这个角度为1rad。两个长度的比值是纯数1，但对应的角度是1rad，数1对应量1rad，但是1不等于1rad，数不等于量。
...
```
国际计量大会的约定：将质量、长度、时间、温度、电流、物质的量、发光强度这7个量作为基本物理量，并没有将“角”作为基本物理量——那么“角”（以平面角为例）这个量如何用基本物理量导出呢？答案是，规定角就是弧长与半径之比，即theta=l/r，从而平面角的标准单位就是米/米即1，也就是说平面角无量纲，但实际上在很多情况下用弧度rad来代替这个“1”是很有必要的，比如“1 rad/s”便清楚地提示这是一个角速度或圆频率，而不是频率或转速。
```
...
如果一个物理问题中有n个未知量，其中有独立量纲的物理量有k个，那么该问题就可以等价转换为关于n-k个无量纲数的问题。
...
具体表述为：如果一个物理问题中有n个未知量，其中有独立量纲的物理量有k个，那么该问题就可以等价转换为关于n-k个无量纲数的问题。
...
3. 量纲共有$2$ 个：面积 $cm^2$、长度 $cm$。

   - **弧度没有量纲**，因为它是长度与长度的比值（弧度定义为圆的弧长比半径）。而弧度可以转化为角度（2π（rad）＝360°），所以**角度亦无量纲**

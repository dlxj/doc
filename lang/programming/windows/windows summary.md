...
磁盘管理 -> 右键 -> 转换为 GPT 就可以了



```

Windows无法安装到这个磁盘,选中的磁盘采用gpt分区形式解决方法。
...
选择 “开始”，键入 diskmgmt.msc，然后选择 Enter。
...
若要验证转换过程，请右键单击磁盘，选择“ 属性”，选择“ 卷 ”选项卡，并检查分区样式信息。
...
```
在硬盘分区格式化界面时按Shift+F10出现命令提示符：

这个时候就进入到了具有管理员权限的CMD窗口。
...
4.clean（清除磁盘所有分区）
5.convert gpt（将磁盘转换成GPT格式）根据实际的需要如果是legacy启动的就转成mbr格式
...
需要声卡 副机连声卡,然后声卡的 otg 口连接主机(打游戏的)。 声音推理都在副机进行.
